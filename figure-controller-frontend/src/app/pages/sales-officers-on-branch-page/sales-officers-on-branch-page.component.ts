...
import { SalesOfficersByBranchResponseDTO, UserService } from '../../services/user.service';
...
@Component({
  selector: 'app-sales-officers-on-branch-page',
  imports: [
  ...
  templateUrl: './sales-officers-on-branch-page.component.html',
  styleUrl: './sales-officers-on-branch-page.component.scss'
})
export class SalesOfficersOnBranchPageComponent implements OnInit {
  branchCode: string = '';
  branchName: string = '';
  fromDate: Date | null = new Date();
  toDate: Date | null = new Date();
  dataSource: SalesOfficersByBranchResponseDTO[] = [];
  isLoading = false;
  ...
  tableConfig: TableConfig = {
          ...
          emptyMessage: 'No Sales Officers data available',
          emptyIcon: 'cancel',
          rowActions: [
            {
              label: 'View GWP',
  ...
  ngOnInit(): void {
    this.branchCode = this.route.snapshot.paramMap.get('branchCode') || '';
  ...
  onSearch() {
    ...
    this.userService.getSFCCodesByBranch( branchCode, fromDate,toDate)
      .subscribe({
        next: (data) => {
  ...
  openGwpDialog(row: SalesOfficersByBranchResponseDTO) {
      if (!this.fromDate || !this.toDate) return;
  ...
  openCollectionDialog(row: SalesOfficersByBranchResponseDTO) {
    if (!this.fromDate || !this.toDate) return;
  ...
  openRenewalDialog(row: SalesOfficersByBranchResponseDTO) {
    if (!this.fromDate || !this.toDate) return;
  ...
  openCancellationDialog(row: SalesOfficersByBranchResponseDTO) {
      if (!this.fromDate || !this.toDate) return;
  ...
  openBusinessDialog(row: SalesOfficersByBranchResponseDTO) {
      if (!this.fromDate || !this.toDate) return;
  ...
  openPerformanceDialog(row: SalesOfficersByBranchResponseDTO) {
      if (!this.fromDate || !this.toDate) return;
